...
class Cifra_Cesar_Descifrar:
  ...
  def subtrair(self):
    """ Subtrai os numeros os mantendo entre 26 e 0 """
    for i in range(len(self.listaInt[0])):
      if((self.listaInt[0][i] - self.listaInt[1][i]) < 0):
        self.resultadoInt.append(26 - ((self.listaInt[0][i] - self.listaInt[1][i])*(-1)))
      else:
